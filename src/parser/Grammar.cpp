...
Parses strings in the language generated by
<OptionalSign> -> "+"|"-"| E   */
bool Grammar::optionalSign(bool& negate)
{
	switch (nextTokenType())
	{
	case MP_PLUS:
...
bool Grammar::simpleExpression(SemanticRecord& record)
{
	...
	bool logged = false;
	bool negate = false; // will be set true in OptionalSign if there is a negative sign
	...
	case MP_TRUE:
		...
		// optional negation
		if (negate){
			if (simpExp_rec.showNextOperand().type() == IntData){
				_semanticAnalyser->writeCommand("NEGS");
			}
			else {
				_semanticAnalyser->writeCommand("NEGSF");
			}
		}
